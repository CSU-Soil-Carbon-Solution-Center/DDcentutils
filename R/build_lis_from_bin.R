...
build_lis_from_bin <- function(site,
  ...
  lis_args_exp  <- paste(scen,    siteRun, TEMPfile)
  ...
  if (length(missing) > 0) {
    stop("DDlist100 did not produce expected .lis file(s): ", paste(missing, collapse = ", "),
         "\nCheck run names (eq/base/scen), working directory, and bin outputs.")
  }
  ...
  if (keep_combined) {
    # nice file stem
    stem <- paste0(site, "_", scen)

    utils::write.table(lis_df,
                       file = file.path(out_dir, scen, paste0(stem, "_eqBaseExp.lis")),
                       row.names = FALSE)
    utils::write.table(lis_df_base_exp,
                       file = file.path(out_dir, scen, paste0(stem, "_baseExp.lis")),
                       row.names = FALSE)

    file.copy(lis_file_exp, file.path(out_dir, scen ,lis_file_exp), overwrite = TRUE)
  }
