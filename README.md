...
#### Choosing the local or API backend
...
``` r
\dontrun{
local_config <- daycent_runner_config(
  backend = "exe", exe_path = "path/to/DayCent",
  dc_path100 = "path/to/100libraryfiles/dc.100", validate_paths = TRUE
)
DayCentRunSite("siteA", "scenario1", config = local_config)
}
```
...
``` r
\dontrun{
api_config <- daycent_runner_config(
  backend = "api",
...
completed <- DayCentRunSite(
  site = "siteA", scen = "scenario1", run_eq = TRUE,
  config = api_config, project_path = "path/to/project", wait = TRUE
)
}
```

Use `wait = FALSE` to submit without polling or downloading. The returned
submission contains `run_id`, which can be resumed later:

``` r
\dontrun{
submitted <- DayCentRunSite(
  site = "siteA", scen = "scenario1", config = api_config,
  project_path = "path/to/project", wait = FALSE
)
status <- watch_daycent_run(api_config, submitted$run_id,
                            timeout_seconds = 3600)
results <- download_daycent_results(
  api_config, submitted$run_id, project_path = "path/to/project",
  keep_zip = FALSE, overwrite = FALSE
)
}
```

API results are copied to `sites/{site}/outputs/{scenario}/`. API metadata
(`status.json`, `run_config.json`, `input_qc.json`, and `input_qc.md`) is
archived under `apiDocs/{timestamp}_{run-id}/`; repeated runs do not silently
replace metadata. Temporary result ZIPs are removed unless `keep_zip = TRUE`.

#### API contract evidence

The offline contract comparison used checked-out `platform/emdc-platform`
revision `d4d00d61fd70f3a32ef2b11504d60e9aa7a8158f`, specifically
`dotnet/src/Web/Endpoints/ModelRuns.cs` and `modeling/README-API.md`. The
observed contract is `POST /api/ModelRuns` with `X-Api-Key`, `productId`,
optional organization/name/site/scenario fields, repeated `include`,
`runEquilibrium`, optional dry-run fields, and `inputZip`. Status is read from
`GET /api/ModelRuns/{runId}` and the complete archive from
`GET /api/ModelRuns/{runId}/download`. The checked-out documentation lists
`Queued`, `Running`, `Completed`, `Failed`, `ValidationFailed`,
`InsufficientCredits`, `PreflightPassed`, and `Cancelled`; production
confirmation of additional client-accepted variants belongs to Packet 08.
No authenticated request, dry run, licensed executable, or live model
